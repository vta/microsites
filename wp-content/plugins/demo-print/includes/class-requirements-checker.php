...
if ( ! class_exists( 'Requirements_Checker' ) ) {
	...
	class Requirements_Checker {
		 ...
		 *
		 * @param string $demo_print Name of the plugin.
		 * @param string $plugin_slug Slug of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_plugin_active( $demo_print, $plugin_slug ) {
			...
			$this->add_error_notice(
				$demo_print . ' is a required plugin.',
				$demo_print . ' needs to be installed & activated.'
			);
		 ...
		 *
		 * @param string $demo_print Demo Print.
		 * @param string $plugin_slug Plugin Slug.
		 * @param string $min_plugin_version Minimum version required of the plugin.
		 * @return boolean
		 * @since 1.0.0
		 */
		private function is_required_plugin_version_active( $demo_print, $plugin_slug, $min_plugin_version ) {
			...
			$this->add_error_notice(
				"{$demo_print} {$min_plugin_version}+ is required.",
				"{$demo_print} {$installed_plugin_version} is installed."
			);
		...
		private function are_required_plugins_dependency_met() {
			...
			$installed_plugins = array_filter(
				...
				function( $required_plugin_data, $required_demo_print ) {
					return $this->is_plugin_active( $required_demo_print, $required_plugin_data['plugin_slug'] );
				},
			...
			$plugins_installed_with_required_version = array_filter(
				$installed_plugins,
				function( $required_plugin_data, $required_demo_print ) {
					return $this->is_required_plugin_version_active( $required_demo_print, $required_plugin_data['plugin_slug'], $required_plugin_data['min_plugin_version'] );
				},
				ARRAY_FILTER_USE_BOTH
			);
