...
if ( ! class_exists( 'Dependency_Loader' ) ) {
	...
	class Dependency_Loader {
		...
		public function load_dependencies( $class ) {
			...
			if ( 'Demo_Print' !== $parts[0] ) {
				return;
			}

			// Remove 'Demo_Print' from parts.
			array_shift( $parts );
			...
			$file_path = Demo_Print::get_plugin_path() . implode( '/', $parts ) . $class_file_name;
			...
			$file_path = Demo_Print::get_plugin_path() . implode( '/', $parts ) . $trait_file_name;
		...
		protected function load_registries() {
			require_once( Demo_Print::get_plugin_path() . 'core/registry/trait-base-registry.php' );
			require_once( Demo_Print::get_plugin_path() . 'core/registry/class-controller.php' );
			require_once( Demo_Print::get_plugin_path() . 'core/registry/class-model.php' );
		}
		...
		protected function load_core() {
			...
			foreach ( glob( Demo_Print::get_plugin_path() . 'core/*.php' ) as $path ) {
				require_once $path;
			}
		}
